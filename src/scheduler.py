...
# --- CONFIGURATION (Driven by environment variables via src.config) ---
TOWER_ID = config.TOWER_ID
START_HOUR_EDT = config.START_HOUR
END_HOUR_EDT = config.END_HOUR
SCAN_DURATION_MINUTES = config.SCAN_DURATION_MINUTES
...
def generate_sampling_schedules() -> List[Dict]:
    ...
    """
    schedules = []
    # Note: The date follows from the context provided in initial setup.
    current_time = datetime.datetime(2026, 8, 3, START_HOUR_EDT, 0, 0)
